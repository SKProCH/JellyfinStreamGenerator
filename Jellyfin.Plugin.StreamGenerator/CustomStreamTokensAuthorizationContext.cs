...
public partial class CustomStreamTokensAuthorizationContext(IAuthorizationContext inner, IUserManager userManager) : IAuthorizationContext
{
    ...
    private Task<AuthorizationInfo>? TryAuthorizeByToken(HttpRequest request)
    {
        ...
        if (!config.StreamTokens.TryGetValue(apiKey.ToString(), out var token) || token.IsExpired())
            return null;
    ...
    [GeneratedRegex("^/Videos/([0-9a-f]{32})/(master|main)\\.m3u8$", RegexOptions.IgnoreCase)]
